...
const Index = () => {
  ...
  return <Layout lastSyncText={getLastSyncText()} onRefresh={refreshEvents}>
      ...
      <div ref={contentRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onScroll={handleScroll} className="">
        {/* Pull-to-refresh indicator */}
        <motion.div className="flex justify-center items-center h-16 pointer-events-none" style={{
        ...
        <Tabs defaultValue="events" className="w-full">
          ...
          <TabsContent value="events" className="mt-0">
            <EventTabs activeTab={activeTab} setActiveTab={setActiveTab} pendingEvents={pendingEvents} confirmedEvents={confirmedEvents} onConfirm={handleConfirmEvent} onEdit={handleEditEvent} onReject={handleRejectEvent} />
          </TabsContent>
          
          <TabsContent value="calendar" className="mt-0">
            <CalendarView events={events} currentDate={currentDate} onDateChange={setCurrentDate} onEventClick={handleCalendarEventClick} />
          </TabsContent>
        </Tabs>
      </div>
      
      <EventForm event={selectedEvent} open={showForm} onClose={() => {
      setShowForm(false);
      setSelectedEvent(null);
    }} onSave={handleSaveEvent} />
      
      <AnimatePresence>
        {showNotification && pendingEvents.length > 0 && <NotificationBubble event={pendingEvents[0]} onOpen={() => {
